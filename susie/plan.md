...
1. 스크립트 표시 (텍스트 + TTS 재생 버튼)
   - tts-generate Edge Function 호출: Storage 캐시 확인 → 없으면 생성 → 서명 URL 반환
   - TTS 로드 실패 시: 에러 아이콘 표시, 텍스트만으로 진행 가능
   - 스크립트 화면 진입 시 다음 스크립트 pre-fetch (백그라운드 캐싱으로 콜드 딜레이 완화)
2. 사용자 차례 → 음성 녹음
   - AppState 리스너: 백그라운드 전환 감지 → 명시적 중단 + 재시작 유도 UI
   - 60초 도달 10초 전부터 카운트다운. 60초에 자동 중단.
3. STT + 발음 평가 Edge Function 호출
   - stt-transcribe: Whisper STT → 텍스트 반환. 10초 타임아웃. 실패 시 에러 + "다시 시도" 버튼
   - pronunciation-score: 원문(Edge Function에서 DB 직접 조회) vs 인식 텍스트 비교 → 0~100 점수 계산
   - STT 결과 먼저 DB 저장 → GPT 피드백은 비동기 분리
4. 결과 표시: pronunciation-score (0~100) 표시. MVP 기준 60점 이상 = 완료 처리. 재도전 무제한 허용.
...
- `status = 'completed'` 조건: 해당 레슨의 모든 스크립트(speaker='user')에서 pronunciation-score 60점 이상 달성 (재도전 무제한)
...
│   │   │   ├── react-native-svg                # v1.1 추가 예정 (MVP에서는 드롭다운 사용)
│   │   │     ├── [v1.1] 국가별 Path/Polygon으로 구성된 SVG 지도 (Maps API 미사용)
│   │   │     ├── [v1.1] 탭 시 Reanimated로 선택 애니메이션
│   │   │     └── [v1.1] 동일 언어 국가(스페인어 권역 등) → 같은 언어 ID로 매핑
...
│
├── [AI / 음성]
│   ├── OpenAI Whisper API                  # STT, 다국어. 텍스트 변환 후 pronunciation-score로 평가
│   ├── OpenAI GPT-4o-mini                 # 표현 수정 제안, 학습 피드백 (비동기)
│   ├── OpenAI TTS                          # 학습 스크립트 AI 음성 (On-demand 생성 + Storage 캐시)
│   │     └── tts-generate Edge Function: 캐시 확인 → 없으면 생성 → Storage 저장 → 서명 URL 반환
│   │           캐시 경로: tts-cache/{language_id}/{lesson_script_id}.mp3 (스크립트당 최초 1회만 생성)
...
| `lesson_scripts` | 스텝별 대화 스크립트 | `audio_storage_path` 컬럼: On-demand TTS 캐시 경로 (null=미생성, 값 있음=캐시됨). tts-generate가 첫 생성 후 업데이트 |
...
[소통]
supabase.storage.from('user-recordings').upload()
supabase.functions.invoke('tts-generate', { lesson_script_id })  # On-demand TTS + 캐시 (학습 스크립트 재생 시)
...
| 함수명 | 트리거 | 주요 동작 |
|--------|--------|----------|
| `tts-generate` | 클라이언트 invoke | Storage 캐시 확인 → 없으면 OpenAI TTS 생성 → tts-cache 저장 → 서명 URL 반환. lesson_script_id 기반 Rate Limit 적용 |
| `stt-transcribe` | 클라이언트 invoke | Whisper STT → 텍스트 반환 → DB 저장 |
| `pronunciation-score` | stt-transcribe 완료 후 invoke | 원문(DB 직접 조회, 클라이언트 전달 금지) vs 인식 텍스트 비교 → 0~100 점수 → user_lesson_progress 저장 |
...
- [ ] Storage: private bucket + `get-signed-url` / `tts-generate` Edge Function으로만 발급
- [ ] tts-cache 버킷: 반드시 private 설정 (public 금지 — 외부 hotlink로 bandwidth 비용 착취 방지)
- [ ] tts-generate: lesson_script_id (UUID) 기반 캐시 경로 생성. 클라이언트에서 임의 텍스트 전달 금지
- [ ] pronunciation-score: 원문 텍스트 클라이언트 전달 금지. Edge Function에서 lesson_script_id로 DB 직접 조회 (점수 조작 방지)
...
| OpenAI TTS (On-demand + 캐시) | $15~40 | 스크립트당 최초 1회만 API 호출, 이후 Storage 캐시 재사용 |
...
> - 학습 TTS는 On-demand + Storage 캐시 구조. 스크립트당 최초 1회만 API 호출, 이후 캐시 재사용
...
- [ ] RLS 정책 설정
- [ ] Storage 버킷 생성 (tts-cache: private, user-recordings: private, avatars: public)
...
| 우선순위 | 기능 | 이유 |
|---------|------|------|
| P1 | SVG 세계 지도 구현 (온보딩) | MVP에서 드롭다운으로 대체된 SVG 지도 구현. 지원 언어 확장 시 신규 국가 Path 추가 |
