...
export default function OnboardingLanguage() {
  ...
  const handleContinue = async () => {
    ...
    const { error } = await supabase
      .from('users')
      .update({ learning_language_id: selected.id, onboarding_step: 1 })
      .eq('id', user.id);
    setSaving(false);
  ...
  return (
    <View className="flex-1 bg-white">
      ...
      <View className="pt-14 px-6 pb-4">
        ...
        <Text className="text-sm font-medium text-gray-400">1 / 3단계</Text>
        <Text className="text-2xl font-bold text-gray-900 mt-1">
          어떤 언어를 배우고 싶으세요?
        </Text>
        <Text className="text-gray-500 mt-1">학습할 언어를 선택해주세요</Text>
      </View>
